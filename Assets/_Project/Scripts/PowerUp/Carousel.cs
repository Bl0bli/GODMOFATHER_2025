    ...
    IEnumerator SpinRoutine(PowerUp target, float spinDuration, Player player)
{
    ...
    int targetIndex = System.Array.IndexOf(_powerUps, target);
    if (targetIndex < 0)
    {
        Debug.LogError("❌ Target PowerUp non trouvé dans la liste !");
        yield break;
    }
    ...
    float targetY = 0f; // centre local
    RectTransform targetItem = _itemList[targetIndex];

    // 🔹 Animation d'apparition (scale 0 -> 1.5)
    ...
    _globalLayout.localScale = Vector3.one * 0.005f;

    // 🔹 Phase de spin principal
    ...
    while (!stopped)
    {
        ...
        if (speed < 30f)
        {
            foreach (var item in _itemList)
                item.localPosition += new Vector3(0f, distance * 0.1f, 0f);
        }
        ...
        if (Mathf.Abs(distance) < 2f && speed < 5f)
        {
            ...
            foreach (var item in _itemList)
                item.localPosition += new Vector3(0f, snapDelta, 0f);
    ...
    // 🔹 Animation disparition (scale 1.5 -> 0)
    ...
    while (disappearElapsed < disappearDuration)
    {
        ...
        disappearElapsed += Time.deltaTime;
        yield return null;
    }
    _globalLayout.localScale = Vector3.zero;
}
